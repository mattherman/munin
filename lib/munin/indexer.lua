...
local function index_directory(repo, path, extensions_to_match)
    ...
    for file in lfs.dir(path) do
        ...
        if not starts_with(file, ".") then
            ...
            logger.trace('file_path = '..file_path)
            local file_attr, error_msg = lfs.attributes(file_path)
            if error_msg then
                logger.warn("Skipping "..file_path..": "..error_msg)
            end
...
function M.index(repo, file_extensions)
    logger.trace("Starting to index "..repo._path)
    local created_backup = false
    local existing_db, _ = lfs.attributes(repo._db_path)
    if existing_db then
        logger.trace("Creating backup at "..repo._backup_path.."...")
    ...
    logger.trace("Creating new database...")
    ...
    if created_backup then
        logger.trace("Deleting backup database at "..repo._backup_path.."...")
    ...
    logger.trace("Indexing complete!")
end
